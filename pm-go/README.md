...
The virtual-store materializer stages complete package entries before atomic publication, with bounded transient rename retries and cleanup on failure or cancellation. Scoped sibling links, nested local targets, graph-hashed identities and warm dependency-link repairs use the reference’s path conventions; Windows junctions target the final location before the staging rename. Tests cover real Node resolution, concurrent publication, failed-CAS cleanup and cold/warm Rust tree snapshots. The project layout, quarantine and state layers described below consume the materializer; install CLI integration remains pending.
...
The hoisted linker plans real package directories with root-direct version priority, consumer-count tie-breaking, nested conflicts, all three hoisting limits and shared workspace roots. Members outside the root keep their own resolvable trees. Parents finish before deeper placements replace bundled files. Driver-vouched complete entries preserve build output, and the placement map records every installed copy for bins and scripts. Tests compare cold/warm file trees, counts and placement maps with Rust. Automatic phantom detection and CLI integration of install-state reuse remain pending.
...
The state differential probe compiles the private Rust state declarations and filesystem verification functions extracted at test build time. A type bridge parses original manifest bytes once into the engine’s serde crate identity; the reference algorithms are retained. Its corpus compares compact state/freshness bytes, malformed-input acceptance and installed-layout reasons; it is never linked into the Go executable.
...
Nub setting defaults now inspect the supplied root/workspace manifests for injected dependencies and version-gated store compatibility. Resolved layout settings feed the actual isolated and hoisted linkers, including shared-store selection, hidden/public hoisting, custom modules paths and strict CLI import-method validation. Tests materialize each layout and inspect its resolved package paths. Automatic filesystem probe placement, full session discovery and phantom-closure policy remain separate integration work.

The settings file view shares npmrc discovery with registry authentication but retains separate user/project tiers. Under pnpm 11, it applies the reference key allowlist while retaining Nub-owned layout keys; extending to a workspace member only adds that member's project tier. Nub's 15 unsupported engine settings cannot re-enter through CLI, environment or file aliases. Their existing refusal advice is retained for config commands; command wiring remains pending.

Storage configuration resolves cache, CAS and versioned global-tree paths once per invocation. Native path overrides, the existing cache environment override, settings, platform directories and HOME-less temporary defaults retain their precedence under the Go namespace. Only an unwritable default store selects the existing project-local fallback; the global CAS remains a read-only fallback. Explicit custom stores retain their errors. Complete installer wiring is still pending.
