...
fn main() {
    ...
    if args.len() == 2 && args[0] == "managed-toml" {
        managed_oracle::run(std::path::Path::new(&args[1]));
        return;
    }
    if args.len() == 2 && args[0] == "install-policy" {
        install_policy_oracle::run(std::path::Path::new(&args[1]));
        return;
    }
    ...
    if args.len() == 2 && args[0] == "manifest-shapes" {
        state_inputs_oracle::manifests(std::path::Path::new(&args[1]));
        return;
    }
    ...
    if args.len() == 2 && args[0] == "resolve" {
        resolve_oracle::run(std::path::Path::new(&args[1]));
        return;
    }
