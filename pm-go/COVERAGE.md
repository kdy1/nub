...
| Project and config | PM identity/major override rules, source-scoped npmrc/auth/TLS/proxies, ordered manifests, install-shape digests, workspace glob matching, Nub shared-store/hoist materialization selection and contradiction checks | Complete settings session, nub.jsonc consumers, Yarn/Bun adapters, workspace discovery and filters |
