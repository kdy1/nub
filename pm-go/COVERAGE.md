...
| Project and config | PM identity/major override rules, source-scoped npmrc/auth/TLS/proxies, ordered manifests, install-shape digests, workspace glob matching, Nub shared-store/hoist materialization selection and contradiction checks; checked 152-setting catalog, invocation-owned scalar/list resolution, source precedence and managed hardening, additive managed TOML file loading, scoped npmrc/global YAML source bundles and raw workspace YAML validation; native install-block validation, field overlays and identity-scoped lowering; manifest-dependent defaults and resolved linker-plan inputs; frozen/force/fix/lockfile-only priority, dependency selection, network policy and ordered CLI setting bags | Complete session assembly, complete nub.jsonc loading, Yarn/Bun adapters, workspace discovery and filters |
