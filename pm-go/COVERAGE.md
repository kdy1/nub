...
| Network and store | HTTP/TLS/auth/redirect limits, metadata caching/offline, SRI, safe archives, BLAKE3 CAS/indexes, process locks, directory/tarball imports, Git ref resolution and commit-verified clone caching with offline reuse, atomic codeload trees and integrity sidecars; registry graph-to-store fetching, frozen URL checks, integrity/content policy, URL-keyed computed-integrity bindings, alias deduplication and offline/corrupt-cache recovery | Batch/source fetch coordinator, maintenance commands, installation state |
